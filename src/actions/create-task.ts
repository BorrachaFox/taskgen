...
export async function runCreateTask() {
    ...
    let linearTemplates: { id: string; name: string; description: string }[] = [];
    ...
    if (hasLinear && teamId) {
        ...
        if (assignToOther) {
            ...
            if (others.length === 0) {
                console.log(
                    chalk.yellow("No other members found in the team. Keeping you as the assignee.")
                );
    ...
    try {
        task = await generateTask(
            { what, changes, impact, type, priority, template: templateContent },
            config
        );
